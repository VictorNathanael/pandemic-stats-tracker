...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['selectCountry'])) {
    ...
    $stmt = $conexao->prepare("INSERT INTO acessos (pais, data, hora) VALUES (?, ?, ?)");
    $stmt->bind_param("sss", $country, $current_date, $current_hour);
    $stmt->execute();}


    $last_country_stmt = $conexao->prepare("SELECT pais FROM acessos ORDER BY id DESC LIMIT 1");
    $last_country_stmt->execute();
    $last_country_result = $last_country_stmt->get_result();
    
    if ($last_country_result->num_rows > 0) {
    ...
    $last_access_stmt = $conexao->prepare("SELECT data, hora FROM acessos ORDER BY data DESC, hora DESC LIMIT 1");
    $last_access_stmt->execute();
    $last_access_result = $last_access_stmt->get_result();

    if ($last_access_result->num_rows > 0) {
